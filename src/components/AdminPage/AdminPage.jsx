...
function AdminPage() {
    ...
    const refreshFeedback = () => {
        axios({
            ...
            console.log('latest feedback:', response.data[response.data.length - 1].id);
    ...
    return (
        ...
        <div>
            <animated.div style={props}>
                ...
                <h4>As of <i>this moment</i>, anonymous feedback has been left <span id="totalFeedback"><u>{totalFeedback}</u></span> times. Leave yours <span id="here" onClick={() => goToPage1()}><b>here</b></span>!</h4>
                    ...
                    <tbody>
                        {feedbackArray.map((feedback, i) => (
                            <tr key={i}>
                                ...
                                <td className={feedbackColor(feedback.feeling)}>{feedback.feeling}</td>
                                <td className={feedbackColor(feedback.understanding)}>{feedback.understanding}</td>
                                <td className={feedbackColor(feedback.support)}>{feedback.support}</td>
                                <td>{feedback.comments}</td>
                                <td>{feedback.flagged ? <div className="yes"><b>Yes</b></div> : <div className="no">No</div>}
                                </td>
                                <td id="flagButtonCell">
                                    {feedback.flagged
                                        ?
                                        <button id="flagButton" onClick={() => switchFlag(feedback.id)}>Un-Flag</button>
                                        :
                                        <button id="flagButton" onClick={() => switchFlag(feedback.id)}>Flag</button>}</td>
                                <td id="dateCell">{new Date(feedback.date).toISOString().slice(0, 10).replace(`T`, ``)}</td>
                                <td><button id="deleteButton" onClick={() => deleteWarn(feedback.id)}>Delete</button></td>
                            </tr>
                        ))}
                    </tbody>
                </table><br />
